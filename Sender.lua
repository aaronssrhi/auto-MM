...
-- Función para verificar si el servidor VIP existe
local function isVIPServerValid(link)
    local code = string.match(link, "roblox.com/share%?code=(%w+)")
    if not code then return false end

    local success, response = pcall(function()
        return HttpService:GetAsync("https://games.roblox.com/v1/games?universeIds="..code)
    end)
...
-- Función para simular la pausa total solo para el jugador
